...
import frappe
from frappe import _, msgprint, scrub, throw
...
def execute(filters=None):
    ...
    if filters.get("group_by_party") and filters.get("group_by_territory"):
        frappe.throw("No se puede seleccionar ambas agrupaciones")

    if filters.get("resumen") and (not filters.get("group_by_territory") and not filters.get("group_by_party")):
        frappe.throw("Se requiere una agrupacion para realizar un resumen")
...
class ReceivablePayableReport(object):
    ...
    def run(self, args):
        ...
        print("\n\n\n\n\n\n")
        print(self.only_subtotal)

        if self.filters.get("resumen"):
            self.data = self.only_subtotal
    ...
    def set_defaults(self):
        ...
        if self.filters.get("group_by_party") or self.filters.get("group_by_territory"):
            self.previous_party = ""
        ...
        for gle in self.gl_entries:
            ...
            if not key in self.voucher_balance:
                self.voucher_balance[key] = frappe._dict(
                    ...
                    remarks=gle.remarks if self.filters.get(
                        "show_remarks") else None,
                    invoiced=0.0,
                    paid=0.0,
                    credit_note=0.0,
                    outstanding=0.0,
            ...
            if self.filters.get("group_by_territory"):
                self.init_subtotal_row(
                    self.get_party_details(gle.party).territory)
    ...
    def update_voucher_balance(self, gle):
        ...
        gle_balance_in_account_currency = self.get_gle_balance_in_account_currency(
            gle)
    ...
    def update_sub_total_row(self, row, party):
        ...
        # self.only_subtotal.append(total_row)
    ...
    def append_subtotal_row(self, party):
        ...
        if sub_total_row:
            ...
            self.only_subtotal.append(sub_total_row)
            self.data.append({})
        ...
        if gle.against_voucher:
            ...
            if gle.against_voucher_type in ("Sales Invoice", "Purchase Invoice"):
                if gle.against_voucher in self.return_entries:
                    return_against = self.return_entries.get(
                        gle.against_voucher)
                    if return_against:
    ...
    def build_data(self):
        ...
        for key, row in self.voucher_balance.items():
            ...
            if (abs(row.outstanding) > 1.0 / 10**self.currency_precision) and (
                    abs(row.outstanding_in_account_currency) > 1.0 /
                10**self.currency_precision
            ):
        ...
        if self.filters.get("group_by_party") or self.filters.get("group_by_territory"):
            ...
            if self.filters.get("group_by_territory"):
                self.append_subtotal_row(self.previous_territory)

            if self.data:
                self.data.append(self.total_row_map.get("Total"))
                self.only_subtotal.append(self.total_row_map.get("Total"))
    ...
    def append_row(self, row):
        ...
        if self.filters.get("group_by_territory"):
            self.update_sub_total_row(row, row.territory)
            if self.previous_territory and (self.previous_territory != row.territory):
                self.append_subtotal_row(self.previous_territory)
            self.previous_territory = row.territory
    ...
    def build_delivery_note_map(self):
        if self.invoices and self.filters.show_delivery_notes:
            ...
            for d in si_against_dn:
                if d.delivery_note:
                    self.delivery_notes.setdefault(
                        d.parent, set()).add(d.delivery_note)
            ...
            for d in dn_against_si:
                self.delivery_notes.setdefault(
                    d.against_sales_invoice, set()).add(d.parent)
    ...
    def get_invoice_details(self):
        ...
        if self.party_type == "Customer":
            si_list = frappe.db.sql(
                """
				select name, due_date, po_no , housing
    ...
    def allocate_outstanding_based_on_payment_terms(self, row):
        ...
        row.payment_terms = sorted(
            row.payment_terms, key=lambda x: x["due_date"])
    ...
    def append_payment_term(self, row, d, term):
        if (
            ...
            invoiced = flt(flt(d.payment_amount) *
                           flt(d.conversion_rate), self.currency_precision)
    ...
    def get_future_payments(self):
        if self.filters.show_future_payments:
            ...
            future_payments = list(
                self.get_future_payments_from_payment_entry())
            ...
            if future_payments:
                for d in future_payments:
                    if d.future_amount and d.invoice_no:
                        self.future_payments.setdefault(
                            (d.invoice_no, d.party), []).append(d)
    ...
    def get_ageing_data(self, entry_date, row):
        ...
        for i, days in enumerate(
                [self.filters.range1, self.filters.range2,
                    self.filters.range3, self.filters.range4]
        ):
    ...
    def get_gl_entries(self):
        ...
        self.gl_entries = frappe.db.sql(
			...
			from
 			...
 			left join tabCustomer tc on tc.name  = gl.party
    ...
    def get_sales_invoices_or_customers_based_on_sales_person(self):
        if self.filters.get("sales_person"):
            ...
            for d in records:
                self.sales_person_records.setdefault(
                    d.parenttype, set()).add(d.parent)
    ...
    def get_order_by_condition(self):
        ...
        elif self.filters.get("group_by_territory"):
            return "order by tc.territory, posting_date"
    ...
    def add_customer_filters(self, conditions, values):
        if self.filters.get("customer_group"):
            conditions.append(self.get_hierarchical_filters(
    ...
    def get_hierarchical_filters(self, doctype, key):
        lft, rgt = frappe.db.get_value(
            doctype, self.filters.get(key), ["lft", "rgt"])
                ...
                if self.filters.get(dimension.fieldname):
                    if frappe.get_cached_value("DocType", dimension.document_type, "is_tree"):
                        self.filters[dimension.fieldname] = get_dimension_with_children(
                            dimension.document_type, self.filters.get(
                                dimension.fieldname)
                        )
    ...
    def get_columns(self):
        ...
        if self.filters.party_type == "Customer":
            ...
            self.add_column(
                label=_("Sector"), fieldname="territory", fieldtype="Link", options="Territory"
            )
        ...
        self.add_column(label=_("Cost Center"),
                        fieldname="cost_center", fieldtype="Data")
        self.add_column(label=_("Voucher Type"),
                        fieldname="voucher_type", fieldtype="Data")
        self.add_column(
            label=_("Voucher No"),
        ...
        if self.filters.show_remarks:
            self.add_column(label=_("Remarks"),
                            fieldname="remarks", fieldtype="Text", width=200),

        self.add_column(label="Due Date", fieldtype="Date")

        if self.party_type == "Supplier":
            self.add_column(label=_("Bill No"),
                            fieldname="bill_no", fieldtype="Data")
            self.add_column(label=_("Bill Date"),
                            fieldname="bill_date", fieldtype="Date")

        if self.filters.based_on_payment_terms:
            self.add_column(label=_("Payment Term"),
                            fieldname="payment_term", fieldtype="Data")
            self.add_column(label=_("Invoice Grand Total"),
        ...
        if self.filters.show_future_payments:
            self.add_column(label=_("Future Payment Ref"),
                            fieldname="future_ref", fieldtype="Data")
            self.add_column(label=_("Future Payment Amount"),
                            fieldname="future_amount")
            self.add_column(label=_("Remaining Balance"),
                            fieldname="remaining_balance")

        if self.filters.party_type == "Customer":
            self.add_column(label=_("Customer LPO"),
                            fieldname="po_no", fieldtype="Data")

            # comma separated list of linked delivery notes
            if self.filters.show_delivery_notes:
                self.add_column(label=_("Delivery Notes"),
                                fieldname="delivery_notes", fieldtype="Data")
            ...
            if self.filters.show_sales_person:
                self.add_column(label=_("Sales Person"),
                                fieldname="sales_person", fieldtype="Data")
    ...
    def add_column(self, label, fieldname=None, fieldtype="Currency", options=None, width=120):
        ...
        self.columns.append(
            dict(label=label, fieldname=fieldname,
    ...
    def setup_ageing_columns(self):
        ...
        self.add_column(label=_("Age (Days)"), fieldname="age",
    ...
    def get_chart_data(self):
        ...
        for row in self.data:
            ...
            if not cint(row.bold):
                values = [row.range1, row.range2,
                          row.range3, row.range4, row.range5]
                precision = cint(frappe.db.get_default("float_precision")) or 2
                rows.append({"values": [flt(val, precision)
                            for val in values]})
